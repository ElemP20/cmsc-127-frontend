...
const PerSemester_Math = ({ courses, term, sem, onCourseSelect, getAvailableCourses, selectedCourses }) => {
  ...
  console.log(remainingCourses, categories);
  return (  
    <TableContainer>
      <Table size="small">
        <TableHead>
        ...
        <TableBody>
          ...
          {term === "1ST YEAR" && sem === "2nd Semester" && (
            <CourseDropdown
            type="HIST 1/KAS 1"
          ...
          {term === "2ND YEAR" && sem === "2nd Semester" && (
            <>
              {categories["FREE ELECTIVE"].map((course) => (
                <StyledTableRow key={course.course_id} status={course.status}>
                  <TableCell align="center">
                    <Stack direction="Column">
                      <Typography align='center' fontSize={'0.8rem'}>{course.course_id}</Typography>
...
export default PerSemester_Math